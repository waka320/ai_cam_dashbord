...
async def analyze_csv_data_debug(csv_path: str, year: int, month: int, purpose: str):
    ...
    debug_responses = {
        ...
        # 日時関連の返答
        "dti_event_effect": f"【デバッグ: {location_name}】{year}年{month}月のイベント効果分析によると、イベント開催日は通常日と比較して歩行者数が約45%増加しています。特に効果が高かったのは14時〜16時の時間帯で、通常の2倍以上の人出がありました。また、イベント翌日も約15%の増加が見られ、波及効果があることも確認できます。さらに土曜開催のイベントは日曜開催より約20%効果が高く、天候が良い日はさらに10%程度効果が増大する傾向にあります。今後のイベント計画には土曜日の午後を中心に検討することをお勧めします。",
        
        "dti_event_time": f"【デバッグ: {location_name}】{year}年{month}月のデータによると、イベント開催に最適な時間帯は13時〜17時です。この時間帯は歩行者数が1日の中で最も多く（1日平均の約40%が集中）、特に14時台がピークとなっています。土日祝日はさらに1時間早く12時から混雑が始まるため、週末のイベントは12時開始が効果的です。また、夕方以降も18時頃まで一定の人出があるため、夕方までイベントを延長すると追加の集客が見込めます。雨天時は全体的に人出が30%程度減少するため、室内イベントの検討も必要です。",
        
        "dti_shift": f"【デバッグ: {location_name}】{year}年{month}月のデータから最適なシフト配置を分析しました。最も人手が必要なのは土曜日と日曜日の13時〜17時で、平日の約2倍の人員が必要です。平日は10時〜12時と15時〜18時の2つのピークがあり、この時間帯に人員を集中させることをお勧めします。特に水曜日は全体的に客足が20%ほど少ないため、研修やミーティングに充てる日として活用できます。月曜午前と金曜夕方も比較的混雑するため、人員配置を厚くする必要があります。",
        
        # 曜日×時間帯関連の返答
        "dwe_open_hour": f"【デバッグ: {location_name}】{year}年{month}月の分析から、最適な営業時間は平日11時〜19時、土日祝10時〜20時です。平日は12時と17時に2つのピークがあり、この時間帯をカバーする営業時間が効率的です。土日は朝から混雑が始まり、10時には平日の正午と同程度の人出があります。また、土日は夕方以降も人出が多く、20時頃まで一定の需要があります。水曜日は特に午前中の人出が少ないため、開店時間を遅らせるか、定休日とすることも検討できます。効率を重視するなら平日11時〜19時、売上最大化を目指すなら全日10時〜20時の営業がおすすめです。",
        
        "dwe_shoping_open_hour": f"【デバッグ: {location_name}】商店街全体の最適な営業時間として、平日は11時〜19時、土日祝は10時〜20時を提案します。{year}年{month}月のデータによると、平日の歩行者数は11時から増加し始め、12時〜13時と17時〜18時に2つのピークがあります。土日は10時から人出が多くなり、13時〜16時がピークとなっています。観光地である高山の特性上、土日は観光客が朝から訪れるため、10時開店が集客に効果的です。また、飲食店は平日でも夜20時まで営業することで、夕食需要を取り込める可能性があります。商店街全体で統一した営業時間を設定することで、来訪者の利便性も向上します。",
        ...
        "dti_cog": f"【デバッグ: {location_name}】{year}年{month}月の日時別混雑度を表示します。平日は12時〜13時と17時〜18時に2つのピークがあり、土日は13時〜16時が最も混雑します。朝9時以前と夜19時以降は全日で混雑度が低くなります。特に水曜日の午前中は週間で最も空いており、混雑度は平均の60%程度です。第2週目の土曜日（{month}/13頃）の14時〜15時は月間で最も混雑する時間帯となっています。平日でも昼休みの12時台は混雑度が高いため、この時間帯の買い物は避けた方が良いでしょう。",
        
        "dwe_cog": f"【デバッグ: {location_name}】曜日×時間帯別の混雑度分析です。最も混雑するのは土曜日の14時〜16時で、次いで日曜日の同時間帯です。平日では金曜日の17時〜19時が最も混雑します。全曜日を通じて、9時以前と20時以降は比較的空いています。水曜日は一日を通して最も混雑度が低く、特に午前中は他の曜日の半分程度の混雑度です。曜日別の混雑度は土曜>日曜>金曜>木曜>火曜>月曜>水曜の順となっています。時間帯別では14時〜16時が最も混雑し、次いで12時〜13時、17時〜19時と続きます。買い物が快適にできるのは、水曜日か月曜日の午前中、または平日の19時以降です。"
    }
